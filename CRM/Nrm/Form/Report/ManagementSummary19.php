...
class CRM_Nrm_Form_Report_ManagementSummary19 extends CRM_Report_Form {
  ...
  function preProcess() {
    $this->assign('reportTitle', ts('Microsite Summary Report for 2019'));
    parent::preProcess();
  }
  ...
  function select() {
    ...
    $this->_select = "
       ...
       WHERE DATE(FROM_UNIXTIME(ws.completed)) >= '{$from}' AND DATE(FROM_UNIXTIME(timestamp)) <= '{$to}' AND purl <> '{$microsite}' AND purl LIKE '%{$microsite}') 
       GROUP BY contact_id
       ) as ue
       ) AS num
       UNION
