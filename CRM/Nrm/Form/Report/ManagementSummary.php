...
class CRM_Nrm_Form_Report_ManagementSummary extends CRM_Report_Form {
  ...
  function select() {
    ...
    $this->_select = "
       ...
       FROM {$this->_drupalDatabase}.watchdog_nrm
       WHERE DATE(FROM_UNIXTIME(timestamp)) = DATE(NOW() - INTERVAL 1 DAY)
       AND location LIKE 'chowan2016.com%' AND location NOT LIKE '%.chowan2016.com%' {$urlWhere}
       GROUP BY location ) as loc
       ) as h
       UNION
       ...
       FROM {$this->_drupalDatabase}.watchdog_nrm
       WHERE location LIKE '%chowan2016.com%' AND location NOT LIKE '%.chowan2016.com%' {$urlWhere}
       GROUP BY location ) as loc
       ) as k
       UNION
